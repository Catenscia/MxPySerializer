...
# launch bandit on the mxpyserializer package
# it is mandatory to obtain a valid check
printf "${BLUE}##########\n# Bandit\n##########${NC}\n"
OUTPUT=$(bandit -r mxpyserializer)
echo "${OUTPUT}"
...
# it is mandatory to obtain valid check
printf "${BLUE}##########\n# ruff\n##########${NC}\n"
OUTPUT=$(ruff format mxpyserializer)
OUTPUT=$(ruff check mxpyserializer)
echo "${OUTPUT}"
...
# launch pylint only on the mxpyserializer package
# score should be equal or above 9.5
printf "${BLUE}##########\n# Pylint\n##########${NC}\n"
OUTPUT=$(pylint mxpyserializer)
echo "${OUTPUT}"
SCORE=$(sed -n '$s/[^0-9]*\([0-9.]*\).*/\1/p' <<< "$OUTPUT")
TEST=$(echo "${SCORE} < 9.5" |bc -l)

ERRORS_FILTER=$(echo "$OUTPUT" | grep "^mxpyserializer/.*: [EF][0-9]\+")
if [ ! -z "${ERRORS_FILTER}" ]
then
